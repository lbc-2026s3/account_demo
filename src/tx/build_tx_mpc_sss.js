/**
 * 教学版 MPC-SSS：Shamir’s Secret Sharing + 恢复私钥后单签
 *
 * 与 TSS 对比：
 * - SSS（本文件）：用 shamirs-secret-sharing 做 t-of-n 分片；
 *               签名前用至少 threshold 个分片 combine 出完整私钥，再单签
 * - TSS：sk ≡ sk1+...+skn (mod n)，各方贡献部分签名后合成；签名过程不重组完整私钥
 *
 * 说明：本文件演示的是「密钥分片托管」，不是真正的门限签名；
 *       签名瞬间完整私钥会在内存中出现。
 */
import {
    http,
